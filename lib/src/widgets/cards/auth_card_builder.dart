...
import 'package:k_login/src/utils/constants.dart';
import 'package:k_login/src/utils/dart_helper.dart';

import 'package:k_login/src/utils/matrix.dart';
import 'package:k_login/src/utils/text_field_utils.dart';
import 'package:k_login/src/utils/widget_helper.dart';
import 'package:k_login/src/widgets/animated_button.dart';
import 'package:k_login/src/widgets/animated_icon.dart';
import 'package:k_login/src/widgets/animated_text.dart';
import 'package:k_login/src/widgets/animated_text_form_field.dart';
import 'package:k_login/src/widgets/custom_page_transformer.dart';
import 'package:k_login/src/widgets/expandable_container.dart';
import 'package:k_login/src/widgets/fade_in.dart';
import 'package:k_login/src/widgets/term_of_service_checkbox.dart';
import 'package:font_awesome_flutter/font_awesome_flutter.dart';
import 'package:provider/provider.dart';
import 'package:sign_in_button/sign_in_button.dart';

import '../circular_switch_tab.dart';
...
class AuthCardState extends State<AuthCard> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    _cardOverlayHeightFactorAnimation = Tween<double>(
    ...
    _cardOverlaySizeAndOpacityAnimation = Tween<double>(
      begin: 1.0,
      end: 0,
    ).animate(
      CurvedAnimation(
  ...
  Future<void> _forwardChangeRouteAnimation(GlobalKey cardKey) {
    ...
    _cardSize2AnimationX = Tween<double>(
      begin: 1.0,
      end: heightRatio / cardSizeScaleEnd,
    ).animate(
      CurvedAnimation(
        parent: _routeTransitionController,
    ...
    _cardSize2AnimationY = Tween<double>(
      begin: 1.0,
      end: widthRatio / cardSizeScaleEnd,
    ).animate(
      CurvedAnimation(
        parent: _routeTransitionController,
        curve: const Interval(.72727272, 1, curve: Curves.easeInOutCubic),
  ...
  Widget _buildLoadingAnimator({Widget? child, required ThemeData theme}) {
    ...
    return Stack(children: <Widget>[card, Positioned.fill(child: overlay)]);
  }
    ...
    Future<bool> requireSignUpConfirmation() async {
      final confirmSignupRequired =
          await auth.confirmSignupRequired?.call(
            LoginData(name: auth.email, password: auth.password),
          ) ??
          true;
          ...
          child: _AdditionalSignUpCard(
            ...
            onSubmitCompleted: () async {
              ...
              if (requireSignupConfirmation) {
                ...
                _forwardChangeRouteAnimation(_additionalSignUpCardKey).then((
                  _,
                ) {
                  widget.onSubmitCompleted?.call();
                });
      ...
      case _confirmSignup:
        return _buildLoadingAnimator(
          ...
          child: _ConfirmSignupCard(
            ...
            onBack:
                () =>
                    auth.additionalSignupData == null
                        ? _changeCard(_loginPageIndex)
                        : _changeCard(_additionalSignUpIndex),
            loadingController: formController,
            onSubmitCompleted: () {
